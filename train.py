...
class Graph():
    def __init__(self, is_training=True):
        ...
        with self.graph.as_default():
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        ## Multihead Attention ( vanilla attention)
                            # The causality in the attention is set to False because all the information in the decoder can be used.
                        self.dec = multihead_attention(queries=self.dec, 
                                                        keys=self.enc, 
                                                        num_units=hp.hidden_units, 
                                                        num_heads=hp.num_heads,
            ...
            # T个词 每个词对应一个len(en2idex)向量 最大的值也就是这个预测的词
            # T words, each word corresponds to a len (en2idex) vector, the largest value is the predicted word
            # 这里应该有个 softmax
